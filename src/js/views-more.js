  ...
  CT.views.claude = ({ tab, query }) => {
    ...
    if (!CT.ai.available()) {
      ...
      return `<section class="claude"><div class="page-head"><div><p class="eyebrow">Claude</p><h1>Ask, invent, analyse.</h1></div></div>
        ...
        ${url ? `<a class="btn primary" href="${CT.esc(url)}" target="_blank" rel="noopener">Open CookThis on claude.ai</a>` : '<p class="muted small">Once published, the link to the claude.ai version appears here.</p>'}
